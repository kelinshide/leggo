...
export default function App() {
  ...
  const modelListEntries = useMemo(
    () => Object.entries(modelList.current),
    [count]
  );
  ...
  // 312 这一块是添加那个other1 tab 还没懂是怎么添加来的
  useMemo(() => {
      ...
      <div className="header">
        ...
        >
          ...
          <Menu.SubMenu
            key="list"
            title="模板列表"
            disabled={!modelListEntries.length}
            icon={<ProfileOutlined />}
          >
            {modelListEntries.map(([modelName, schemaModel]) => (
              <Menu.Item
                key={modelName}
                onClick={() => setSelectedModel(schemaModel)}
              >
                {modelName}
              </Menu.Item>
            ))}
          </Menu.SubMenu>
          <Menu.Item key="readme" icon={<SolutionOutlined />}>
            项目介绍
          </Menu.Item>
        </Menu>
      </div>
    ...
    <div className="multi-cover">
      ...
      <div className="multi-cover-list">
        ...
        {dataList.map((src: string, index: number) => (
          <div
            key={src + index}
            className="multi-cover-list-item"
            onClick={() => pickPic(index)}
          >
            {src && (
              <div className="multi-cover-list-item-mask">
                <span>替换</span>
              </div>
            )}
            {src ? (
              <img src={src} alt="封面图" width="100%" />
            ) : (
              <PlusOutlined />
            )}
          </div>
